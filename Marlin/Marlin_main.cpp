...
#if HAS_BED_PROBE
  float zprobe_zoffset = Z_PROBE_OFFSET_FROM_EXTRUDER; // Initialized by settings.load()
#endif
  ...
  void update_software_endstops(const AxisEnum axis) {
      ...
      if (axis == X_AXIS) {
        ...
        if (active_extruder != 0) {
          // T1 can move from X2_MIN_POS to X2_MAX_POS
          ...
          // T0 can move from X1_MIN_POS to X1_MAX_POS
...
inline void gcode_G4() {
  ...
  inline void home_z_safely() {
     ...
     */
    destination[X_AXIS] = Z_HOMING_X_POINT;
    destination[Y_AXIS] = Z_HOMING_Y_POINT;
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    set_destination_from_current();
      ...
      if (home_all || homeZ) homeaxis(Z_AXIS);
        ...
        if (z_homing_height && (home_all || homeZ)) {
    ...
    SERIAL_PROTOCOLLNPGM("About to home Z...");
...
inline void gcode_M115() {
  ...
  #ifdef SHORT_BUILD_VERSION
    SERIAL_PROTOCOLLNPGM("RAPIDIA:FIRMWARE_VERSION:" SHORT_BUILD_VERSION);
  #endif
