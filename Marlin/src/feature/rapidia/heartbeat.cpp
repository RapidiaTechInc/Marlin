...
namespace Rapidia
{
  ...
  HeartbeatSelectionUint Heartbeat::selection = static_cast<HeartbeatSelectionUint>(HeartbeatSelection::_DEFAULT);
...
#define ES_REPORT(S, N)          \
  if (endstops.endstop_state(S)) \
    SERIAL_CHAR_CHK(N);
